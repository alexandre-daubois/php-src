...
<?php
    ...
    try {
        ...
        if (is_object(prepex(29, $db, 'CREATE FULLTEXT INDEX idx1 ON test_prepare_emulated(label)'))) {
            ...
            prepex(31, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES (1, :placeholder)',
                array(':placeholder' => 'If I have the freedom to choose, I would always go again for the MySQL Server'));
            $stmt = prepex(32, $db, 'SELECT id, label FROM test_prepare_emulated WHERE MATCH label AGAINST (:placeholder)',
        ...
        prepex(35, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES (1, :placeholder), (2, :placeholder)',
            array(':placeholder' => 'row'));
/*
        $stmt = prepex(36, $db, 'SELECT id, label FROM "test_prepare_emulated WHERE MATCH label AGAINST (:placeholder)',
            array(':placeholder' => 'row'),
            array('execute' => array('sqlstate' => '42000', 'mysql' => 1064)));
*/
        $stmt = prepex(37, $db, 'SELECT id, label FROM \'test_prepare_emulated WHERE MATCH label AGAINST (:placeholder)',
            array(':placeholder' => 'row'),
            array('execute' => array('sqlstate' => '42000', 'mysql' => 1064)));

        $stmt = prepex(38, $db, 'SELECT id, label AS "label" FROM test_prepare_emulated WHERE label = :placeholder',
            array(':placeholder' => 'row'));

        $sql = sprintf("SELECT id, label FROM test_prepare_emulated WHERE (label LIKE %s) AND (id = :placeholder)",
            $db->quote('%ro%'));
        ...
        $sql = sprintf("SELECT id, label FROM test_prepare_emulated WHERE  (id = :placeholder) OR (label LIKE %s)",
            $db->quote('%ro%'));
        $stmt = prepex(41, $db, $sql,	array('placeholder' => 1));
        if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 2)
                printf("[042] Expecting two rows, got %d rows\n", $tmp);

        $sql = "SELECT id, label FROM test_prepare_emulated WHERE id = :placeholder AND label = (SELECT label AS 'SELECT' FROM test_prepare_emulated WHERE id = :placeholder)";
        ...
        prepex(49, $db, 'DELETE FROM test_prepare_emulated');
        prepex(50, $db, "INSERT INTO test_prepare_emulated(id, label) VALUES(1, '?')",
            array('first row'));
        $stmt = prepex(51, $db, 'SELECT label FROM test_prepare_emulated');

        var_dump($stmt->fetchAll(PDO::FETCH_ASSOC));
        prepex(52, $db, 'DELETE FROM test_prepare_emulated');
        prepex(53, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES(1, ?)',
            array('first row'));
        prepex(54, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES(2, ?)',
            array('second row'));
        $stmt = prepex(55, $db, 'SELECT label FROM test_prepare_emulated');
        var_dump($stmt->fetchAll(PDO::FETCH_ASSOC));

        // Is PDO fun?
        prepex(56, $db, 'SELECT label FROM test_prepare_emulated WHERE ? > 1',
            array('id'));
        prepex(57, $db, 'SELECT ? FROM test_prepare_emulated WHERE id > 1',
            array('id'));
        prepex(58, $db, 'SELECT ? FROM test_prepare_emulated WHERE ? > ?',
            array('test'), array('execute' => array('sqlstate' => 'HY093')));

        prepex(59, $db, 'SELECT ? FROM test_prepare_emulated WHERE ? > ?',
        ...
        for ($num_params = 2; $num_params < 100; $num_params++) {
            ...
            prepex(60, $db, 'SELECT id, label FROM test_prepare_emulated WHERE label > ?',
                $params, array('execute' => array('sqlstate' => 'HY093')));
        }

        prepex(61, $db, 'DELETE FROM test_prepare_emulated');
        prepex(62, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES (1, ?), (2, ?)',
            array('row', 'row'));
        $stmt = prepex(63, $db, 'SELECT id, label FROM test_prepare_emulated');
        var_dump($stmt->fetchAll(PDO::FETCH_ASSOC));

        $stmt = prepex(64, $db, 'SELECT id, label FROM test_prepare_emulated WHERE ? IS NOT NULL',
            array(1));
        if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 2)
            printf("[065] '1' IS NOT NULL evaluates to true, expecting two rows, got %d rows\n", $tmp);

        $stmt = prepex(66, $db, 'SELECT id, label FROM test_prepare_emulated WHERE ? IS NULL',
            array(1));
        if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 0)
            printf("[067] '1' IS NOT NULL evaluates to true, expecting zero rows, got %d rows\n", $tmp);

        prepex(68, $db, 'DROP TABLE IF EXISTS test_prepare_emulated');
        prepex(69, $db, 'CREATE TABLE test_prepare_emulated(id INT, label CHAR(255)) ENGINE=MyISAM');
        if (is_object(prepex(70, $db, 'CREATE FULLTEXT INDEX idx1 ON test_prepare_emulated(label)'))) {
            prepex(71, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES (1, ?)',
                array('MySQL is the best database in the world!'));
            prepex(72, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES (1, ?)',
                array('If I have the freedom to choose, I would always go again for the MySQL Server'));
            $stmt = prepex(73, $db, 'SELECT id, label FROM test_prepare_emulated WHERE MATCH label AGAINST (?)',
                array('mysql'));
            /*
            Lets ignore that
            if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 2)
        ...
        prepex(74, $db, 'DELETE FROM test_prepare_emulated');
        prepex(75, $db, 'INSERT INTO test_prepare_emulated(id, label) VALUES (1, ?), (2, ?)',
            array('row', 'row'));

        $stmt = prepex(76, $db, 'SELECT id, label FROM "test_prepare_emulated WHERE MATCH label AGAINST (?)',
            array('row'),
            array('execute' => array('sqlstate' => '42000', 'mysql' => 1064)));
        ...
        */
        ...
        $stmt = prepex(78, $db, 'SELECT id, label AS "label" FROM test_prepare_emulated WHERE label = ?',
            array('row'));

        $sql = sprintf("SELECT id, label FROM test_prepare_emulated WHERE (label LIKE %s) AND (id = ?)",
            $db->quote('%ro%'));
        ...
        $sql = sprintf("SELECT id, label FROM test_prepare_emulated WHERE  (id = ?) OR (label LIKE %s)",
            $db->quote('%ro%'));
        $stmt = prepex(81, $db, $sql,	array(1));
        if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 2)
                printf("[082] Expecting two rows, got %d rows\n", $tmp);

        $sql = "SELECT id, label FROM test_prepare_emulated WHERE id = ? AND label = (SELECT label AS 'SELECT' FROM test_prepare_emulated WHERE id = ?)";
        $stmt = prepex(83, $db, $sql,	array(1, 1));
        if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 1)
                printf("[084] Expecting one row, got %d rows\n", $tmp);

        $sql = "SELECT id, label FROM test_prepare_emulated WHERE id = :placeholder AND label = (SELECT label AS 'SELECT' FROM test_prepare_emulated WHERE id = ?)";
